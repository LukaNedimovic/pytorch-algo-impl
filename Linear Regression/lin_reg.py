...
def forward(X): 
    ...
    """
    return w * X # Function is a simple linear equation: f(x) = w * x
...
if __name__ == "__main__":
    ...
    y_data = 3 * x_data # The function the model is using to predict w and b
    ...
    for epoch in range(epochs):
        ...
        # Backward pass
        loss.backward()                               # Calculate the gradient w.r.t "w"     
        w.data = w.data - learning_rate * w.grad.data # Make a step opposite of gradient to make error smaller
        w.grad.data.zero_()                           # Zero out the gradient                          

        print("Epoch: {}, \tW: {}, \tLoss {}:".format(epoch, w.item(), loss.item())) # Print the data


    # Calculate the final prediction
    final_prediction = w.item() * x_data # "w" now has the final predicted value
